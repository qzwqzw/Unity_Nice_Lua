...
public class GestureControl : MonoBehaviour
{
   ...
   void LateUpdate()
   {
      ...
      if (target)
      {
         ...
         // target.transform.LookAt(new Vector3(-pos.x, 0 ,-pos.z));
         // var axis = ETCInput.instance.axes as ETCAxis;
         // target.transform.Translate( axis.GetInfluencedAxis() * axis.axisSpeedValue , Space.World);
         
         // //获取EasyTouch的摇杆坐标
         // ETCJoystick joystick = ETCInput.GetControlJoystick("Joystick");

         // //人物转向
         // target.transform.LookAt(transform.position + new Vector3(joystick.axisX.axisValue, 0 , joystick.axisY.axisValue));
